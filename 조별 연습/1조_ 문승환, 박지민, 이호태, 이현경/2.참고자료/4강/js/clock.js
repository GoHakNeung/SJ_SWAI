class Clock {
  ...
  render() {
    ...
    clockBox.setAttribute("style", `width:${this.width}px;height:${this.height}px`);
    ...
    this.hourHand.setAttribute("fill", "#ff0000");
    this.hourHand.setAttribute(
      "points",
    ...
    this.secondHand.setAttribute("fill", "black");
    this.secondHand.setAttribute(
      "points",
      "191,193 193,193 193,35 191,35"
    );
    ...
    if(n==1){
        var clockRun = setInterval(()=>{
          console.log(this.second)
            if(((this.second%60)==0)&&(this.second/60)>0) this.minute++;
            if(((this.minute%60)==0)&&(this.minute/60)>0)this.hour++;
            this.updateClockHands()
        },1000)
    }
    else if(n==2) {
